        ...
        <div style={{ height: 54, display: "flex", alignItems: "center", padding: "0 14px", gap: 4, flexShrink: 0, background: WB_PANEL, borderBottom: `1px solid ${BORDER}`, boxShadow: "0 1px 8px rgba(0,0,0,.04)", zIndex: 30 }}>
            ...
            {isHost && (
                <div style={{ display: "flex", gap: 1, marginLeft: 2, padding: "2px", borderRadius: 9, border: `1px solid ${BORDER}`, background: "#F8F5F0" }}>
                    <button className="tb-btn" onClick={onUndo} title="Undo" style={{ padding: 6, borderRadius: 7, border: "none", background: "transparent", color: TSUB, cursor: "pointer", display: "flex" }}><Undo2 size={15} strokeWidth={2} /></button>
                    <button className="tb-btn" onClick={onRedo} title="Redo" style={{ padding: 6, borderRadius: 7, border: "none", background: "transparent", color: TSUB, cursor: "pointer", display: "flex" }}><Redo2 size={15} strokeWidth={2} /></button>
                </div>
            )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <button onClick={onCopyRoomId} title="Copy Room ID"
